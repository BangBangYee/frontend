...
function MainPage() {
  return (
    <div className="flex flex-col items-center justify-center h-[823px] w-[386px] mx-auto mx-auto bg-green-100" style={{ position: 'relative' }}>
      {/* 상단 카메라와 마이페이지 아이콘 */}
      <div className="flex justify-between w-full p-4" style={{ position: 'absolute', top: 0 }}>
        <img src={cameraIcon} alt="Camera Icon" className="w-8 h-8 cursor-pointer" />
      ...
      <div className="w-full bg-white rounded-t-3xl pt-8 px-6 shadow-md mt-8 mb-0" style={{ height: '100%' }}>
        ...
        <div className="mb-4 p-4 bg-green-100 rounded-lg flex items-center">
          ...
          <div>
            ...
            <button className="mt-2 px-4 py-1 text-sm text-green-500 border border-green-500 rounded-full bg-white">주파수 분석중 <span>&gt;</span></button>
          </div>
        </div>
        ...
        <div className="p-4 bg-red-100 rounded-lg flex items-center border-2 border-red-400 mb-4">
          ...
          <div>
            ...
            <button className="mt-2 px-4 py-1 text-sm text-red-500 border border-red-500 rounded-full bg-white">자세 감지중 <span>&gt;</span></button>
          </div>
        </div>
        {/* 하단 아이 보기 버튼 */}
        <button className="w-full py-4 text-white bg-green-500 rounded-md hover:bg-green-600 focus:outline-none focus:bg-green-700">
          아이 보러가기
        </button>
      </div>
